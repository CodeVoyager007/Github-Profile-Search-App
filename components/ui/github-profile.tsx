...
export default function GitHubProfileViewer() {
  ...
  const fetchUserData = async (): Promise<void> => {
    ...
    try {
      ...
      const profileData: UserProfile = await profileResponse.json(); // Cast to UserProfile type
      ...
      const reposData: UserRepo[] = await reposResponse.json(); // Cast to UserRepo array type
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black py-8 relative">
      <Card className="w-full max-w-3xl p-6 space-y-4 shadow-lg rounded-lg bg-black border border-gold">
        <CardHeader className="text-center">
          <CardTitle className="text-3xl font-bold mb-2 text-white">
            GitHub Profile Viewer
          </CardTitle>
          <CardDescription className="text-gray-400">
            Search for a GitHub username and view their profile and repositories, search like: CodeVoyager007
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit} className="mb-8 px-6">
          <div className="flex items-center gap-4">
            ...
            >
              {loading ? <ClipLoader className="w-4 h-4 text-black" /> : "Search"}
            </Button>
          </div>
        </form>
            ...
            <div className="grid md:grid-cols-[120px_1fr] gap-6">
              ...
              <div className="grid gap-2">
                <div className="flex items-center gap-2">
                  <h2 className="text-2xl font-bold text-white">{userProfile.login}</h2>
                  <Link
                ...
                <div className="flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-white">{userProfile.followers} Followers</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <UsersIcon className="w-4 h-4 text-white" />
                    <span className="text-white">{userProfile.following} Following</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <LocateIcon className="w-4 h-4 text-white" />
                    <span className="text-white">{userProfile.location || "N/A"}</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="grid gap-6">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6">
                {userRepos.map((repo) => (
                  <Card
                    key={repo.id}
                    className="shadow-md rounded-lg bg-white border border-silver transition-transform transform hover:scale-105 hover:shadow-xl"
                  >
                    <CardHeader>
                      <div className="flex items-center gap-2">
                    ...
                    <CardFooter className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-sm text-gray-300">
                        ...
                        <ForkliftIcon className="w-4 h-4 text-gold" />
      ...
      {/* Tagline positioned in the bottom right corner with maroon heart */}
